...
class ControlsSubState extends MusicBeatSubstate
{
	...
	var keyArray:Array<FlxKey> = [
		CoolUtil.leftKeybind,
		CoolUtil.downKeybind,
		CoolUtil.upKeybind,
		CoolUtil.rightKeybind
	];
	...
	public function new()
	{
		...
		var left:FlxText = new FlxText(20, 20, 0, Std.string(keyArray[0]), 32);
		add(left);
		optionArray.insert(0, left);
		var down:FlxText = new FlxText(20, left.y + 50, 0, Std.string(keyArray[1]), 32);
		add(down);
		optionArray.insert(1, down);
		var up:FlxText = new FlxText(20, down.y + 50, 0, Std.string(keyArray[2]), 32);
		add(up);
		optionArray.insert(2, up);
		var right:FlxText = new FlxText(20, up.y + 50, 0, Std.string(keyArray[3]), 32);
		add(right);
	...
	override public function update(elapsed)
	{
		...
		if (waitingForInput)
		{
			if (!blockedKeys.contains(FlxG.keys.firstJustPressed()))
			{
				...
				optionArray[curSelected].text = Std.string(keyArray[curSelected]);
